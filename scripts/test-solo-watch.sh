...
if [[ $SC == "S" || $SC == "s" || $SC == "" ]]; then
  ...
  TEST_COMMAND="test-server:watch" TEST_FILE=$1 docker-compose -f docker-compose-test-solo.yml up --abort-on-container-exit
fi

if [[ $SC == "C" || $SC == "c" ]]; then
  echo -e "${GREEN}\nGotcha. Looking for test files matching \"$1\" in the client${NC}\n"
  TEST_COMMAND="test-client:watch" TEST_FILE=$1 docker-compose -f docker-compose-test-solo.yml up --abort-on-container-exit
fi
